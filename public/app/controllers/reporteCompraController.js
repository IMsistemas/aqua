    ...
    $scope.initLoad = function(){
        ...
        $http.get(API_URL + 'reportecompra/getCompras?filter=' + JSON.stringify(filter)).success(function(response){
            ...
            for (var i = 0; i < longitud; i++) {
                $scope.totalsubconimp += parseFloat(response[0].subtotalconimpuestocompra);
                $scope.totalsubsinimp += parseFloat(response[0].subtotalsinimpuestocompra);
                $scope.totalsubcero += parseFloat(response[0].subtotalcerocompra);
                $scope.totalsubnoobj += parseFloat(response[0].subtotalnoobjivacompra);
                $scope.totalsubex += parseFloat(response[0].subtotalexentivacompra);
                $scope.totaliva += parseFloat(response[0].ivacompra);
                $scope.totalice += parseFloat(response[0].icecompra);
                $scope.totaldesc += parseFloat(response[0].totaldescuento);
                $scope.total += parseFloat(response[0].valortotalcompra);
            }
