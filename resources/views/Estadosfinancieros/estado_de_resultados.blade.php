	...
	<div class="col-xs-12">
		<table class="table table-bordered">
			...
			<tbody>
				...
				$aux_total_ingreso=0;
				$aux_cont=0;
				foreach ($estador["Ingreso"] as $item):?>
		 			<tr>
		 				<td class=""><?= $item["aux_jerarquia"] ?></td>
		 				<td class=""><?= $item["concepto"] ?></td>
		 				<td class="text-right"><?php 
		 					if($aux_cont==0){
		 						$aux_total_ingreso=$item["balance"];
		 					}
				...
				$aux_total_costo=0;
				$aux_cont2=0;
				foreach ($estador["Costo"] as $item1):?>
		 			<tr>
		 				<td class=""><?= $item1["aux_jerarquia"] ?></td>
		 				<td class=""><?= $item1["concepto"] ?></td>
		 				<td class="text-right"><?php 
		 					if($aux_cont2==0){
		 						$aux_total_costo=$item1["balance"];
		 					}
				...
				$aux_total_gasto=0;
				$aux_cont3=0;
				foreach ($estador["Gasto"] as $item2):?>
		 			<tr>
		 				<td class=""><?= $item2["aux_jerarquia"] ?></td>
		 				<td class=""><?= $item2["concepto"] ?></td>
		 				<td class="text-right"><?php 
		 					if($aux_cont3==0){
		 						$aux_total_gasto=$item2["balance"];
		 					}
